...
writer = None
cnt = 0
npzFrame=np.zeros((1,17,2))
while(cap.isOpened()):
    ret, frame = cap.read()
    if frame is None:
        break

    posed_frame, candidate, subset = process_frame(frame, body=not args.no_body,hands=not args.no_hands)
...
'''
    if cnt == 1:
       vid = candidate[:,0:2].reshape(1,17,2)
    else:
       vid = np.concatenate((vid,candidate[:,0:2].reshape(1,17,2)),axis=0)
    '''
